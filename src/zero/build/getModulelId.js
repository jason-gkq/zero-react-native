...
let buzModuleIdMap = [];
const fs = require('fs');
const platformMapPath = `${paths.appBuild}${pathSep}platformMap.json`;
// const buzMapPath;
// var baseMappingPath;
var buzMappingPath;
...
function getModuleIdByIndex(projectRootPath, path, entry, isBuz) {
  ...
  //   baseMappingPath = `${paths.appBuild}${pathSep}platformMap.json`;
  // }
  if (baseModuleIdMap.length === 0) {
    if (fs.existsSync(platformMapPath)) {
      baseModuleIdMap = require(platformMapPath);
      if (baseModuleIdMap.length !== 0) {
  ...
  if (isBuz) {
    if (buzMappingPath == null) {
      buzMappingPath = `${paths.appBuild}${pathSep}${entry.replace(
        '.js',
        'Map.json'
      )}`;
    }
  ...
  if (findPlatformItem) {
    ...
    if (!isBuz) {
      ...
      fs.writeFileSync(platformMapPath, JSON.stringify(baseModuleIdMap));
      return curModuleId;
    } else {
      //业务包
